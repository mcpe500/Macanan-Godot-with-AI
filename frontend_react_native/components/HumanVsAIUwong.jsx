...
const HumanVsAIUwongGame = () => {
  const {
    board,
    message,
    uwongPawnsInHand,
    uwongTotal,
    handleClick, // Human player (Macan) uses this
    handleAIMove, // AI (Uwong) uses this
    win,
    winner,
    goBack,
    restartGame,
    ...
    return indices.map((index) => (
      <View key={index} >
        <BoardCell
          index={index}
          piece={board[index]}
          onPress={handleClick} // Human player (Macan) interacts
          isSelected={selectedPiece === index}
          isDisabled={currentPlayer === 'uwong'} // Disable cells if it's AI's (Uwong) turn
        />
      </View>
    ));
  };
  ...
  return (
    <ScrollView style={styles.screenContainer}>
      <View style={styles.gameContainer}>
        ...
        <Text style={styles.statusText}>Current Player: {currentPlayer.toUpperCase()} ({gameState}) {currentPlayer === 'uwong' && !win ? '(AI Thinking...)' : ''}</Text>
                    ...
                    {mainBoardIndices.map(index => (
                        <View key={index} style={{width: CELL_SIZE, height: CELL_SIZE}}>
                             <BoardCell
                                ...
                                onPress={handleClick}
                                isSelected={selectedPiece === index}
                                isDisabled={currentPlayer === 'uwong'}
                            />
                        </View>
                    ))}
                </View>
...
export default HumanVsAIUwongGame;
